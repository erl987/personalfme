	...
	*/
	namespace EmailTest {
		namespace EmailGatewayTest {
			// Test section
			BOOST_AUTO_TEST_SUITE( EmailGateway_test_suite );

			/**	@brief		Testing of the e-mail gateway
			*/
			BOOST_AUTO_TEST_CASE( EmailGateway_test_case )
			{
				using namespace std;
				unique_ptr< External::CAlarmGateway> gateway = make_unique<External::Email::CEmailGateway>();
