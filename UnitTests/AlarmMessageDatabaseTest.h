...
namespace Networking {
	...
	*/	
	namespace AlarmMessageDatabaseTest {
		...
		const External::Groupalarm::CGroupalarm2Message groupalarmDefault({ {"label 1", 2}, {"label 2", 1} }, { "scenario 1", "scenario 2" }, { "unit 1", "unit 2" }, { std::make_pair<>("Bob", "Foo"), std::make_pair<>("Alice", "Bar") }, "default", "", 2.53);
		...
		const External::Groupalarm::CGroupalarm2Message groupalarmInfo({}, {}, {}, {std::make_pair<>("Bob", "Foo")}, "", "infoalarm_template", 2.53);
		...
		*/
		External::CAlarmMessageDatabase GenerateDatabase(void)
		{
			...
			database.Add( code1, DEFAULT_VALIDITY, make_shared< CGroupalarm2Message >( groupalarmDefault ) );
			database.Add( code1, make_shared< CWeeklyValidity >( weeklyException ), make_shared< CGroupalarm2Message >( groupalarmEmpty ) );
			database.Add( code1, make_shared< CMonthlyValidity >( monthlyException ), make_shared< CGroupalarm2Message >( groupalarmException ) );
			database.Add( code3, DEFAULT_VALIDITY, make_shared< CGroupalarm2Message >( groupalarmDefault ) );
			database.Add( code3, DEFAULT_VALIDITY, make_shared< CEmailMessage>( email ) );

			auto groupalarmDefaultVec = { make_shared< CGroupalarm2Message >( groupalarmDefault ) };
			...
			database.Add(code2, make_shared< CMonthlyValidity > ( monthlyException ), make_shared< CGroupalarm2Message >( groupalarmException ) );
		...
		*/
		External::CAlarmMessageDatabase GenerateDatabaseWithAllAndFallback( void ) {
			...
			auto infoalarmMessages = { make_shared<CInfoalarmMessageDecorator>( make_unique<CGroupalarm2Message>( groupalarmInfo ) ) };
			CAlarmValidities infoalarmDataset( DEFAULT_VALIDITY, begin( infoalarmMessages ), end( infoalarmMessages ) );
			auto allExceptionMessages = { make_shared<CGroupalarm2Message>( groupalarmDefault ) };
			...
			auto fallbackExceptionMessages = { make_shared<CGroupalarm2Message>( groupalarmException ) };
		...
		*/
		BOOST_AUTO_TEST_CASE( AlarmMessageDatabase_add_remove_test_case )
		{
			...
			BOOST_CHECK_THROW( database.Add( emptyCode, DEFAULT_VALIDITY, make_shared< CGroupalarm2Message >( groupalarmDefault ) ), std::length_error );
			BOOST_CHECK_NO_THROW( database.Add( code4, DEFAULT_VALIDITY, make_shared< CGroupalarm2Message >( groupalarmEmpty ) ) );
			...
			BOOST_REQUIRE( *( dynamic_pointer_cast< CGroupalarm2Message >( groupalarmGet4.front() ) ) == groupalarmEmpty );

			shared_ptr< Validities::CValidity > emptyValidity;
			BOOST_CHECK_THROW( database.Add( code4, emptyValidity, make_shared< CGroupalarm2Message >( groupalarmDefault ) ), std::runtime_error );
		...
		*/
		BOOST_AUTO_TEST_CASE( AlarmMessageDatabase_replace_all_fallback_test_case )
		{
			...
			auto fallbackMessages = { make_shared<CGroupalarm2Message>( groupalarmDefault ) };
		...
		*/
		BOOST_AUTO_TEST_CASE( AlarmMessageDatabase_comparison_test_case )
		{
			...
			database2.Add( code3, make_shared< Validities::CWeeklyValidity >( weeklyException ), make_shared< CGroupalarm2Message >( groupalarmDefault ) );
		...
		*/
		BOOST_AUTO_TEST_CASE( AlarmMessageDatabase_search_test_case )
		{
			...
			messageText = dynamic_pointer_cast< CGroupalarm2Message >( database.Search( code1, CBoostStdTimeConverter::ConvertToStdTime( currTime.utc_time() ), isDefaultDataset1 ).front() )->GetMessageText();
			...
			messageText2 = dynamic_pointer_cast< CGroupalarm2Message >( database.Search(code1, CBoostStdTimeConverter::ConvertToStdTime( currTime2.utc_time() ), isDefaultDataset2 ).front() )->GetMessageText();
		...
		*/
		BOOST_AUTO_TEST_CASE( AlarmMessageDatabase_search_all_fallback_test_case )
		{
			...
			BOOST_REQUIRE( foundMessageTypes3.at( typeid( CGroupalarm2Message ) ) == 2 );
			...
			for ( auto const& message4 : messages4 ) {
				string messageText;
				messageText = dynamic_pointer_cast<CGroupalarm2Message>(message4)->GetMessageText();
