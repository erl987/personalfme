...
*	@remarks 										The data size should be at least 100 - 1000. The smaller the datasize, the larger the deviation in length of the calculated time stamps will be (< 2 �s per call of this function)
*/
template <class T> template <class In_It1, class In_It2> void Core::General::CSearch<T>::PutSignalData(In_It1 refTimeFirst, In_It1 refTimeLast, In_It2 signalFirst, In_It2 signalLast)
{
	...
	if ( newCalcSignalTime.size() > 0 ) {
		if ( !( lastCalcTime.is_not_a_date_time() ) ) {
			newCalcSignalTime[0] = lastCalcTime + microseconds( static_cast<long>( 1.0e6 / samplingFreq ) ); // maximum error is 1 �s
		} else {
		...
		if ( newCalcSignalTime.size() > 1 ) {
			...
			for (size_t i=1; i < newCalcSignalTime.size(); i++) {
				newCalcSignalTime[i] = newCalcSignalTime[0] + microseconds( static_cast<long>( i * 1.0e6 / samplingFreq ) ); // maximum error is 1 �s
			}
		}
